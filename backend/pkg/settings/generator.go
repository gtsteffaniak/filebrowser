...
func CollectCommentsAndSecrets(srcPath string) (CommentsMap, SecretFieldsMap, DeprecatedFieldsMap, error) {
	...
	dirsToparse := []string{
		settingsDir,
		filepath.Join(settingsDir, "..", "..", "internal", "database", "users"),
	}
...
func readEmbeddedYaml() (string, error) {
	...
	embeddedContent, err := os.ReadFile("internal/web/dist/config.generated.yaml")
	if err != nil {
		// Try alternative path
		embeddedContent, err = os.ReadFile("backend/internal/web/dist/config.generated.yaml")
		if err != nil {
...
func GenerateYaml() {
	...
	yamlContent, err := GenerateConfigYamlWithSource(&Config, true, true, true, "pkg/settings")
	if err != nil {
...
func GenerateConfigYaml(config *Settings, showComments bool, showFull bool, filterDeprecated bool) (string, error) {
	...
	sourcePaths := []string{
		...
		"../pkg/settings", // When running from backend directory
		"pkg/settings",    // Alternative path
	}
