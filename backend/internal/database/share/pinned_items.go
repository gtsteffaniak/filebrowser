...
func (s *Share) EnsurePinnedItems() PinnedItems {
	if s.PinnedItems == nil {
		s.PinnedItems = make(PinnedItems)
	}
	return s.PinnedItems
}

// ShareRelativeDir maps a source index directory path to a share-relative directory path.
// s.Path is the index path of the share root (e.g. "/share/").
func (s *Share) ShareRelativeDir(indexDirPath string) (string, error) {
	shareRootIndex := utils.AddTrailingSlashIfNotExists(s.Path)
