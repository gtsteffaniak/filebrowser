...
func deleteApiTokenHandler(w http.ResponseWriter, r *http.Request, d *Context) (int, error) {
	name := r.URL.Query().Get("name")
	if !d.User.Permissions.Api {
	...
	activity.RecordTokenMutation(r, toActor(d), activitydb.EventTokenDelete, name)
	return RenderJSON(w, r, response)
}
...
func listApiTokensHandler(w http.ResponseWriter, r *http.Request, d *Context) (int, error) {
	if !d.User.Permissions.Api {
...
func getApiTokenHandler(w http.ResponseWriter, r *http.Request, d *Context) (int, error) {
	name := r.URL.Query().Get("name")
	if !d.User.Permissions.Api {
		return http.StatusForbidden, fmt.Errorf("user does not have permission to list api tokens")
	}
