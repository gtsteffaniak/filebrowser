...
func accessGetHandler(w http.ResponseWriter, r *http.Request, d *Context) (int, error) {
...
func accessPostHandler(w http.ResponseWriter, r *http.Request, d *Context) (int, error) {
	sourceName := r.URL.Query().Get("source")
	indexPath := r.URL.Query().Get("path")
	index := indexing.GetIndex(sourceName)
	if index == nil {
	...
	activity.RecordAccessCreate(r, toActor(d), sourceName, parsedPath.String(), activity.AccessRuleCreateChanges(body.Allow, body.RuleCategory, body.Value))
...
func accessDeleteHandler(w http.ResponseWriter, r *http.Request, d *Context) (int, error) {
	sourceName := r.URL.Query().Get("source")
	indexPath := r.URL.Query().Get("path")
	index := indexing.GetIndex(sourceName)
	if index == nil {
	...
	if cascade {
		...
		activity.RecordAccessDelete(r, toActor(d), sourceName, parsedPath.String(), activity.AccessRuleDeleteChanges(ruleType, ruleCategory, value, true, count))
	...
	activity.RecordAccessDelete(r, toActor(d), sourceName, parsedPath.String(), activity.AccessRuleDeleteChanges(ruleType, ruleCategory, value, false, 0))
...
func groupGetHandler(w http.ResponseWriter, r *http.Request, d *Context) (int, error) {
...
func groupPostHandler(w http.ResponseWriter, r *http.Request, d *Context) (int, error) {
	if !d.User.Permissions.Admin {
		return http.StatusForbidden, nil
	}
	group := r.URL.Query().Get("group")
...
func groupDeleteHandler(w http.ResponseWriter, r *http.Request, d *Context) (int, error) {
	if !d.User.Permissions.Admin {
		return http.StatusForbidden, nil
	}
	group := r.URL.Query().Get("group")
...
func accessPatchHandler(w http.ResponseWriter, r *http.Request, d *Context) (int, error) {
	var body struct {
	...
	activity.RecordAccessUpdate(r, toActor(d), body.Source, newPath.String(), []activitydb.FieldChange{
