...
func TestPrepareActivityItemsForViewerTrimsNonAdminPaths(t *testing.T) {
	viewer := &users.User{
		ID: 2,
	...
	prepareActivityItemsForViewer(items, viewer)
...
func TestPrepareActivityItemsForViewerSkipsAdmin(t *testing.T) {
	admin := &users.User{
		ID: 1,
		FrontendUser: users.FrontendUser{
	...
	prepareActivityItemsForViewer(items, admin)
