...
func New(concurrencyLimit int, ffmpegPath string, cacheDir string) *Service {
	...
	if cacheDir != "" {
		...
		if err != nil {
			...
			logger.Fatal("failed to create file cache path, which is now require to run the server: %v", err)
		}
	} else {
	...
	if ffmpegPath != "" {
		...
		if err != nil {
			logger.Fatal("the configured ffmpeg path does not contain a valid ffmpeg binary %s, err: %v", ffmpegPath, err)
		}
		ffprobePath, err = CheckValidFFprobe(ffmpegPath)
		if err != nil {
			logger.Fatal("the configured ffmpeg path is not a valid ffprobe binary %s, err: %v", ffmpegPath, err)
		}
	}
	return &Service{
...
func GeneratePreview(file iteminfo.ExtendedFileInfo, previewSize, officeUrl string, seekPercentage int) ([]byte, error) {
	...
	if err := service.fileCache.Store(context.Background(), cacheKey, resizedBytes); err != nil {
		logger.Error("failed to cache resized image: %v", err)
	}
...
func DelThumbs(ctx context.Context, file iteminfo.ExtendedFileInfo) {
	...
	if errSmall != nil {
		...
		if errLarge != nil {
			logger.Debug("Could not delete thumbnail: %v", file.Name)
		}
	}
}
