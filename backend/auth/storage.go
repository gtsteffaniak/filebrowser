...
func NewStorage(back StorageBackend, userStore *users.Storage) (*Storage, error) {
	...
	if settings.Config.Auth.TotpSecret != "" {
		...
		if err == nil && (keyLen == 16 || keyLen == 24 || keyLen == 25) {
			// Use the user-provided key if it's valid
			encryptionKey = key
		} else {
			// Otherwise, derive the key from the provided secret string
			log.Println("[WARN] totpSecret is not a valid Base64 encoded key. Deriving a key from it. For better security, generate a secret with 'openssl rand -base64 32'.")
