...
The `scan interval` is the time between scans -- not a cron job that runs exactly on each interval. So, a longer scanning process will take `scan interval` + `scan time` to complete.

A typical scan can vary dramatically. Here is a good rule of thumb for SSD based disks:

| # folders  | time to index  | memory usage (RAM) |
|---|---|---|
|  10,000 | 0-2 seconds | 15 MB |
| 100,000  |  5-30 seconds | 150 MB |
| 500,000  |  1-2 minutes | 500 MB |

### Smart Scanning

1. There is floating `smart scan interval` that ranges from **1 minute - 4 hours** depending on the complexity of your filesystem
...
3. The `smart scan interval` performs a `quick scan` 4 times in a row, followed by a 5th `full scan` which completely rebuilds the index.
   ...
   - A quick scan **cannot** detect when a file has been updated, for example when you safe a file and the size increases. Or when you delete a file and replace it with a new one with the same name.
...
4. The `smart scan interval` changes based on a couple of things. A `simple` complexity enables scans every 1 minute if changes happen frequently. `high` complexity indicates a minimum scanning interval to 10 minutes.
   - The number of directories:
     - **under 10,000** is considered `simple` complexity.
     - **more than 500,000** is always considered `high` complexity.
   - The time it takes to complete a full scan:
     - **Under 2 seconds** is considered `simple` complexity.
     - **Over 2 minutes** is considered `high` complexity.
...
The scan behavior is still 4 quick scans @ given interval, followed by a 5th full scan.
...
I chose this smart indexing method using filesystem scanners because it allows a low footprint design that can cater to individual filesystem complexity. There are a few options for monitoring a filesystem for changes:
...
1. **Option 1**: Recursive Traversal with ReadDir
  ...
  - Low overhead and straight forward implementation.
2. **Option 2**: Use File System Monitoring (Real-Time or Periodic Check) such as `fsnotify`
  ...
  - Requires OS level configuration to ulimits to properly watch a large filesystem.
...
You can see what the index looks like by using the resources API via the GET method, which returns an individual directories information -- all of this information is stored in the index.
...
You can disable the index by setting `indexing: false` in your config file. You will still be able to browse your files, but search will not work at all and you may run into issues as its not intended to be used without indexing.
