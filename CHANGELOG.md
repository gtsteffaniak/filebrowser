 ...
 **Breaking Changes**:
 ...
 - Changed (reverse proxy): `X-Forwarded-For`, `X-Real-IP`, `X-Forwarded-Proto`, and `X-Forwarded-Host` are honored **only** when listed in `http.trustedHeaders`. Upgrades behind nginx, Traefik, or Caddy must add this list or client IP, OIDC callbacks, cookies, subpath URLs, and activity logs may break. FileBrowser logs startup warnings when OIDC is enabled or `baseURL` is not `/` but forwarded headers are not configured. The [config migration tool](https://filebrowserquantum.com/en/docs/getting-started/v2/config-migration/) warns when OIDC is enabled without `X-Forwarded-Proto` / `X-Forwarded-Host`. See [Reverse proxy](https://filebrowserquantum.com/en/docs/getting-started/reverse-proxy/) and [HTTP trustedHeaders](https://filebrowserquantum.com/en/docs/configuration/http/#trustedheaders).
