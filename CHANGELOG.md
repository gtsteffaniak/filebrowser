 ...
 **Security**:
 - [High] Stored XSS via HTML preview: `<script>` is no longer preserved in srcdoc, the preview iframe is opaque-origin (no `allow-same-origin`), the session cookie is HttpOnly, and the SPA shell sends a CSP (GHSA-vvm6-jwrf-hgmg) -- thanks @qrn12580
