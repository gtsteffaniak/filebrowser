<template>
  ...
  <form @submit="save" class="card active" v-if="loaded">
    <div class="card-title">
      ...
      <h2 v-else-if="actor.id == user.id"> {{ $('settings.modifyCurrentUser') }} ({{ user.username }})</h2> <!-- eslint-disable-line @intlify/vue-i18n/no-raw-text -->
      <h2 v-else> {{ $('settings.modifyOtherUser') }} {{ user.username }}</h2>
    </div>
